...
const UserProfile = ({route, navigation}) => {
  ...
  const userList = conversation?.conversation_type == 'group' ? conversation?.participants : [];
      ...
      <View style={styles.container}>
        ...
        <View style={styles.card}>
          {conversation?.conversation_type == 'group' ? ( <>
            ...
            <Text style={styles.userName}>{conversation?.conversation_name}</Text>
            <Text style={styles.userStatus}>{conversation?.participants?.length} Members</Text>
          </>) : (<>
          
          <Image source={user1Img} style={styles.userImage} />
          <Text style={styles.userName}>{user?.first_name} {user?.last_name}</Text>
          <Text style={styles.userStatus}>{user?.status}</Text>
          {user?.user_email ? <Text style={styles.emailText}>{user?.user_email}</Text> : null} 
          </>)}
          <ScrollView style={styles.clubListContainer}>
            {userList?.map((club, index) => (
              ...
              >
                <Text style={styles.clubName}>{club?.user_details?.first_name} {club?.user_details?.last_name}</Text>
